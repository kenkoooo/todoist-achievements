...
class TodoistActivityLoader:
    @staticmethod
    def load(api_token: str, time_difference: datetime.timedelta) -> List[datetime.datetime]:
        api = todoist.TodoistAPI(api_token)
        activity_list = api.activity.get(event_type="completed", since=TODOIST_START, limit=100)
        dates = []
        for activity in activity_list:
            event_date = datetime.datetime.strptime(activity["event_date"], DATE_FORMAT) + time_difference
